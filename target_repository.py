...
class TargetRepository:
    ...
    def _upsert_prj_attribute(self, db, record, user_id, cache=None):
        ...
        if entity is None:
            entity=PrjAttribute(**values,created_by=user_id);db.add(entity)
    ...
    def _upsert_business_logic(self, db, record, user_id, cache=None):
        ...
        if entity is None:
            entity=PrjAttrBusinessLogic(**values,created_by=user_id);db.add(entity)
            if cache is not None: cache[record["prj_id"]]=entity
        else:
            for k,v in values.items(): setattr(entity,k,v)
            entity.version_no=(entity.version_no or 1)+1
    ...
    def _upsert_scope(self, db, record, user_id, cache=None):
        ...
        if entity is None:
            entity=PrjAttrBusinessLogicScope(**values,created_by=user_id);db.add(entity)
            if cache is not None: cache[record["prj_id"]]=entity
        else:
            for k,v in values.items(): setattr(entity,k,v)
            entity.version_no=(entity.version_no or 1)+1
