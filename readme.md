# Data Dictionary Streamlit Admin

## Poetry project layout

This package has been converted to a Poetry-based `src` layout.

```text
data-dictionary-streamlit/
├── pyproject.toml              # Poetry dependency and test configuration
├── src/
...
├── run_local.sh / run_local.bat
└── run_api.sh / run_api.bat
```
...
poetry install
cp .env.example .env
poetry run streamlit run DataDictionaryAdminApp/src/DataDictionaryAdminApp/streamlit_app.py
```

## Run API with Poetry

```bash
poetry run python -m uvicorn DataDictionaryAdminApp.api.swagger_app:app --host 0.0.0.0 --port 8502 --reload
```
...
- Additional SQL Server indexes were added to `infra/sql/01_schema.sql` for portfolio, active-record, PRJ_ID, section, audit, and history lookups.

After replacing the package, rerun `infra/sql/01_schema.sql` once against `PRJ_DB` to add the performance indexes. The script is idempotent and will not recreate tables.


## Restructured package layout

The application package is under `DataDictionaryAdminApp/src/DataDictionaryAdminApp`. SQL scripts are under `infra/DataDictionaryAdminApp/sql`.
...
## Part 3 database and S3 snapshot

Run `infra/DataDictionaryAdminApp/sql/00_create_all_tables.sql` against `PRJ_DB`. It is idempotent and creates the legacy plus Part 3 tables, indexes, and soft-delete columns.

The **Save to S3** action and `POST /api/v1/s3/export` now export a timestamped `full_snapshot` folder containing every mutable table, including inactive/soft-deleted records. Static/reference tables are excluded.
